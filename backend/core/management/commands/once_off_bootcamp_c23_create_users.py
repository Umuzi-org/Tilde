...
from core.models import User, UserGroup, UserGroupMembership, Cohort, RecruitCohort
from social_auth.models import SocialProfile
from curriculum_tracking.models import CourseRegistration, AgileCard
import datetime
now= datetime.datetime.now()
group = UserGroup.objects.get_or_create(name="ds bootcamp sept 20")[0]
cohort = Cohort.objects.get_or_create(cohort_number="23",
start_date = now,
end_date = now,
...
def process_row(row):
    ...
    RecruitCohort.objects.get_or_create(user=user,cohort=cohort,defaults = {'start_date' : now,
'end_date' : now},employer_partner_id=4)


    UserGroupMembership.objects.get_or_create(user=user,group=group,permission_student=True)
    CourseRegistration.objects.get_or_create(user=user,curriculum_id=33,order=1) # intro to tilde
    CourseRegistration.objects.get_or_create(user=user,curriculum_id=20,order=2) # ds pre boot
    CourseRegistration.objects.get_or_create(user=user,curriculum_id=28,order=3) # ds boot
