...
def get_users_and_progress():
    ...
    df_red = df[df["priority"]>CUTTOFF_RED] # these people need 1/1 sessions with experts

    df_orange = df[df["priority"]<=CUTTOFF_RED] # these can be in group sessions
    df_orange = df_orange[df_orange["priority"]>CUTTOFF_ORANGE]

    df_yellow = df[df["priority"]<=CUTTOFF_ORANGE] # these people dont get included if they have another session coming up
...
def hydrate_learner_data(learner_dicts):
    ...
    for d in learner_dicts:
        ...
        existing_sessions = (
            ...
            .filter(is_complete=False).count()
            )
...
def schedule_orange_sessions(orange_learner_dicts, yellow_learner_dicts):
    ...
    yellow_learner_dicts = [d for d in yellow_learner_dicts if d["upcomming_sessions"] == 0]
    ...
    for stream in streams:
        ...
        groups = [stream_learner_dicts[x:x+2] for x in range(0, len(stream_learner_dicts), 2)]
        for group in groups:
            session = Session.objects.create(
                session_type = session_type,
                due_date=timezone.now() + timezone.timedelta(days=DUE_DAYS),
...
def get_progress_df():
    ...
    df = df[["email","how_far_in_program","agile_percent_core_complete","group_for_reporting"]]
    for s in ["strat", "design", "prov","bridge", "devops",]:
        df = df[~df["group_for_reporting"].str.contains(s,case=False)]
    
    df = df[df["how_far_in_program"] > df["agile_percent_core_complete"]]

    df["target"] = df.apply(lambda row: row["agile_percent_core_complete"] / row["how_far_in_program"], axis=1)

    df["time_left"] = df.apply(lambda row: 1-row["how_far_in_program"], axis=1)
    df["priority"] = df.apply(lambda row: (1-row["target"])/row["time_left"] , axis=1)
