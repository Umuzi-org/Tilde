...
def get_project(user,content_title):
    return RecruitProject.objects.filter(content_item__title=content_title, recruit_users__in=[user]).first()
...
def get_github_page_url(user,content_title):
    ...
    found = (re.search("git@github.com:(.*)/(.*).git",url) or 
    re.search("https://github.com/(.*)/(.*)$",url) or
    re.search("https://github.com/(.*)/(.*).git",url))
    if found:
...
def get_card_data(user):
    ...
    cards = AgileCard.objects.filter(assignees__in=[user],content_item__content_type=ContentItem.PROJECT).order_by("order")


    for card in cards:
        tag_names = [o.name for o in list(card.content_flavours.all()) +list(card.content_item.tags.all())]
        ...
        data[f"{card.content_item.title}: start_time"] = card.due_time.strftime(DATE_FORMAT) if card.due_time else None
        data[f"{card.content_item.title}: start_time"] = card.start_time.strftime(DATE_FORMAT) if card.start_time else None
        data[f"{card.content_item.title}: review_request_time"] = card.review_request_time.strftime(DATE_FORMAT) if card.review_request_time else None
        data[f"{card.content_item.title}: complete_time"] = card.complete_time.strftime(DATE_FORMAT) if card.complete_time else None
...
def get_tag_summaries(card_data):
    ...
    for title in card_data['all_project_titles']:
        ...
        for tag in card_data[f"{title}: tags"]:
            ...
            tag_data[f'{tag}_complete_weight'] = tag_data.get(f'{tag}_complete_weight',0)
            tag_data[f'{tag}_incomplete_weight'] = tag_data.get(f'{tag}_incomplete_weight',0)
...
def get_report_data(user,path):
    ...
    portfolio = get_github_page_url(user=user, content_title="Build your first personal website")
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        path = Path(f"gitignore/high_res_report_{group_name}_{datetime.date.today().strftime(DATE_FORMAT)}",)
        os.makedirs(path,exist_ok=True) 

        group = get_group(group_name)

        users = [o.user for o in UserGroupMembership.objects.filter(group = group,permission_student=True)]
