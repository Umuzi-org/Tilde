...
class request_user_can_stop_card_Tests(TestCase):
    def setUp(self):
    ...
    def test_assignee_can_stop_card(self):
        for card in self.cards:
            if card.status == AgileCard.IN_PROGRESS:
                self.assertTrue(card.request_user_can_stop_card(self.assignee_user))
            else:
                self.assertFalse(card.request_user_can_stop_card(self.assignee_user))

    def test_user_with_team_manage_permissions_can_stop_card(self):
        for card in self.cards:
            if card.status == AgileCard.IN_PROGRESS:
                self.assertTrue(
                    card.request_user_can_stop_card(self.user_with_manage_permissions)
                )
            else:
                self.assertFalse(
                    card.request_user_can_stop_card(self.user_with_manage_permissions)
    ...
    def test_superuser_can_stop_card(self):
        for card in self.cards:
            if card.status == AgileCard.IN_PROGRESS:
                self.assertTrue(card.request_user_can_stop_card(self.superuser))
            else:
                self.assertFalse(card.request_user_can_stop_card(self.superuser))

    def test_user_without_team_manage_permissions_cannot_stop_card(
        self,
    ):
        for card in self.cards:
            self.assertFalse(
                card.request_user_can_stop_card(self.user_without_manage_permissions)
            )
