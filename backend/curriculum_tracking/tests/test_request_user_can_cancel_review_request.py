...
class request_user_can_cancel_review_request_Tests(TestCase):
    def setUp(self):
    ...
    def test_assignee_can_cancel_review_request_on_card(self):
        for card in self.cards:
            if card.status == AgileCard.IN_REVIEW:
                self.assertTrue(
                    card.request_user_can_cancel_review_request(self.assignee_user)
                )
            else:
                self.assertFalse(
                    card.request_user_can_cancel_review_request(self.assignee_user)
    ...
    def test_user_with_team_manage_permissions_can_cancel_review_request_on_card(self):
        for card in self.cards:
            if card.status == AgileCard.IN_REVIEW:
                self.assertTrue(
                    card.request_user_can_cancel_review_request(
                        self.user_with_manage_permissions
                    )
                )
            else:
    ...
    def test_superuser_can_cancel_review_request_on_card(self):
        for card in self.cards:
            if card.status == AgileCard.IN_REVIEW:
                self.assertTrue(
                    card.request_user_can_cancel_review_request(self.superuser)
                )
            else:
                self.assertFalse(
                    card.request_user_can_cancel_review_request(self.superuser)
    ...
    def test_user_without_team_manage_permissions_cannot_cancel_review_request_on_card(
        self,
    ):
        for card in self.cards:
            self.assertFalse(
